...
// Support Plans
export async function getSupportPlans(clientId?: number): Promise<SupportPlan[]> {
  const query = clientId ? `?client_id=${clientId}` : '';
  return fetchAPI<SupportPlan[]>(`/api/support-plans${query}`);
}

export async function createSupportPlan(data: Omit<SupportPlan, 'id'>): Promise<SupportPlan> {
...
export async function createCaseRecord(data: Omit<CaseRecord, 'id'>): Promise<CaseRecord> {
...
export async function updateCaseRecord(id: number, data: Partial<CaseRecord>): Promise<CaseRecord> {
...
// Schedules
export async function getSchedules(params?: { client_id?: number; start_date?: string; end_date?: string }): Promise<Schedule[]> {
  const query = params
...
export async function streamAIChat(
  ...
  return fetch(`${BASE_URL}/api/ai/chat`, {
    ...
    body: JSON.stringify({ message, session_id: sessionId, conversation_history: conversationHistory }),
  });
}

export async function generateSupportPlan(clientId: number): Promise<{ plan: string }> {
...
export async function summarizeRecord(recordId: number): Promise<{ summary: string }> {
...
export async function generateReport(clientId: number): Promise<{ report: string }> {
